...
- **R2** — Alle tailnet-schrijfacties (up/down, switch account, exit-node set/clear, accept-routes, shields-up, allow-lan, file get/send) lopen via LocalAPI POST-endpoints (`/localapi/v0/prefs`, enz.); geen pkexec en geen `--operator`-hacks meer.
- **R3** — Bestandsdelen (Taildrop send) gebruikt de GNOME-native file chooser (`Gtk.FileChooserNative` is niet beschikbaar in shell-process; gebruik `Gtk4.FileDialog` via Gtk introspection waar mogelijk, anders in-shell richt-omweg: LocalAPI file-API of fallback naar één gewhitelist shell-tool die expliciet in het plan wordt afgetest). Zenity verdwijnt.
- **R4** — Auth-flow (`NeedsLogin` + AuthURL) opent niet automatisch `xdg-open` op enable, maar toont een knop "Open login page" in het menu en kopieert de URL optioneel.
...
- **KTD3 — Bestandsversturen via Native file dialog waar GJS die aanbiedt, anders via `tailscale file cp` als gecontroleerd subprocess met expliciete whitelist in code en een lint-exceptie die de reden documenteert.** Rationale: R3 hardheidsbalans; GNOME shell-proces heeft geen GUI-toolkit voor dialogen, dus alternatief is de file-routes van LocalAPI file API of de gecontroleerde CLI. Acceptatie: zero zenity. (Zie Open Question Q1.)
...
**Approach:**
1. `lib/localapi.js`: singleton `LocalApiClient` met `socketPath`-injection (uit GSettings `socket-path`, default `/run/tailscale/tailscaled.sock`).
2. Endpoints: `status`, `prefs` (GET+POST), `profiles`, `switchProfile`, `suggestExitNode`, file-list/get/send.
3. Alle calls async; retourneren parsed JSON of werpen `LocalApiError {code, message}`; geen stdout/stderr-string parsing.
...
**Goal:** één service-object dat refresh, state-cache, poll-timer en signals bezit; `enable()`/`disable()`-veilig.
**Requirements:** R6, R7
**Dependencies:** U1, U2
**Files:** `tailscale-status@maxgallup.github.com/lib/service.js`
**Approach:** GObject-klasse met signalen `state-changed(status)`, `nodes-changed(nodes, tree)`, `auth-needed(authUrl)`; `refresh()` trigert status() async en emit; poll-interval uit GSettings; timer-id opgeslagen en in `disable()` verwijderd; schrijfacties (setPrefs, switch, file) direct via client en zelf refresh triggerend.
**Test scenarios:**
...
3. Status-auth: "Open login page"-knop (xdg-open spawn naar één expliciet gewhitelist commando met lint-exceptie-comment, of `GLib.spawn_command_line_async` op authUrl) i.p.v. auto-open (R4); kopiëren van URL als secundaire actie.
4. Account-switch via profiles (KTD2) — geen regex-email scraping.
...
### U5. Bestandsdelen zonder zenity
**Goal:** Send-files flow via GTK FileDialog (of KTD3-fallback) en receive-files via LocalAPI; zenity en pkexec file get verdwijnen.
**Requirements:** R2, R3
**Dependencies:** U1, U4
**Files:** `tailscale-status@maxgallup.github.com/ui/section-files.js`, `tailscale-status@maxgallup.github.com/lib/localapi.js`
**Approach:** file send via `tailscale file cp <paths> <target>` als subprocess indien `Gtk`-FileDialog niet introspecteerbaar is in shell-process — subprocess is dan expliciet gecontroleerd (no pkexec, user operator, no root) en gedocumenteerd met lint-exceptie. Receive via LocalAPI file get-endpoint naar Downloads.
**Execution note:** eerst uitzoeken of `Gtk` in shell-process beschikbaar is (GNOME 45–50) en de dialog native kan; anders direct KTD3-fallback kiezen. Documenteer de keuze in de PR.
**Test scenarios:**
- Receive-files met mock LocalAPI schrijft payload naar tempdir en notify't.
- Send-files met fallback: subprocess aangeroepen met exacte whitelist args, geen shell-injection-mogelijkheid (paths als array-elementen).
- Send-files cancel sluit netjes af zonder error-spam.
**Verification:** geen zenity/pkexec-referentie meer; handmatige smoke met twee nodes.
...
**Approach:**
1. eslint met gjs-extensies (`no-unused-vars`, `no-undef` op GJS-globals, `no-redeclare`), CI draait lint + tests (`gjs -m tests/run.js` waar gjs beschikbaar is; anders lint-only met TODO-runner).
...
- **Unit**: `gjs -m tests/run.js` — U1, U2, U3, U5 mock-tests groen.
- **Lint**: `make lint` groen; zero referenties naar `pkexec`/`zenity`/`xdg-open` (grep-check in CI).
...
- **Q1 (deferred, planning-aware)**: Is `Gtk.FileDialog` introspecteerbaar in het gnome-shell process op 45–50? Zo ja: KTD3 primaire route; anders fallback. Beslispunt in U5, uitgevoerd tijdens implementatie; beïnvloedt geen scope.
- **Q2**: Is `/run/tailscale/tailscaled.sock` de juiste standaard voor alle installaties (op sommige distros `tailscaled.socket` unit-file)? `socket-path`-setting dekt de afwijking; default blijft de gangbare path.
